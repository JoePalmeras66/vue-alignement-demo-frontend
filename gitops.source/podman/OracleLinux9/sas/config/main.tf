...
resource "keycloak_openid_client" uiux {
  client_id   = var.keycloak_client_id
  name        = var.keycloak_client_id
  realm_id    = keycloak_realm.uiux.id
...
resource keycloak_openid_client_authorization_scope GET {
    resource_server_id = keycloak_openid_client.uiux.resource_server_id
    name               = "GET"
    realm_id           = keycloak_realm.uiux.id
}
...
resource "keycloak_role" APP_VIEWER {
  realm_id    = keycloak_realm.uiux.id
  client_id   = keycloak_openid_client.uiux.id
  name        = "APP_VIEWER"
