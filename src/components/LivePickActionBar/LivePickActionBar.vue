...
interface Props {
  ...
  task?: TaskType
  isActionBarTopDisabled?: boolean
}
...
const isShowDetailsDisabled = computed((): boolean => {
  return (
    !hasTask.value ||
    props.disabledButtons?.includes(DisabledButton.edit_scans) ||
    areAllButtonsDisabled.value
...
const showShowDetailsButton = computed((): boolean => {
  return isPickingTask(props.task as TaskType) && !props.showZeroCrossing
})
...
const isButtonDisabled = (buttonAction: ButtonAction) => {
  switch (buttonAction) {
    ...
    case ButtonAction.show_details:
      return isShowDetailsDisabled.value
  }
}
...
const showActionBarTop = computed(() => {
  return !props.isActionBarTopDisabled;
})
...
<template>
  <div class="action-bar" :class="{ 'robot-paused': robotPaused }">
    <div v-if="showActionBarTop" class="action-bar-top">
    ...
    <div class="action-bar-bottom">
      ...
      <div class="content">
        <div class="content-left">
          <IconButton
            v-if="showShowDetailsButton"
            plain
            type="primary"
            class="show-details-button"
            icon="visibility"
            :disabled="isShowDetailsDisabled"
            :text="getActionBarTranslation('show_details')"
            @click="buttonClicked(ButtonAction.show_details)"
          />
          <IconButton
            v-if="robotPaused"
            plain
