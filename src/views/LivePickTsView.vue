...
import LivePickAisle, {
  PropsLivePickLoadCarriers,
} from '@/components/LivePickAisle/LivePickAisle.vue'
...
const targetLoadCarriersAisleLeft = computed<PropsLivePickLoadCarriers[]>(
  () => [
    {
      ref: loadCarrierLeftRef,
      propsLivePickLoadCarrier: {
        headerTextOverride: 'A1',
        pcotsLocation: unref(pcotsLocationLeft),
        loadCarrier: unref(loadCarrierDataLeft),
        task: unref(showTask),
        position: Position.left,
        scannedBarcodes: unref(barcodes),
        showZeroCrossing: unref(showZeroCrossing),
        headerData: unref(loadCarrierHeaderDataLeft),
        sendToReject: unref(sendToRejectLeft),
        showModificationMenu: unref(showLoadCarrierModificationMenu),
        compartmentInfos: unref(compartmentInfosLeft),
        consolidationMode: unref(consolidationMode.value),
        loadCarrierTypes: unref(loadCarrierTypes.value),
        workstationMode: unref(workstationMode),
        countedCompartments: getCountedCompartments(unref(pcotsLocationLeft)),
        showViewContent: false,
        showDummyLoadCarrier: getShowDummyLoadCarrier(unref(pcotsLocationLeft)),
        isRovoflexPicking: unref(showRovoflexComponents),
        showLoadCarrierHeader: true,
        showLoadCarrierNumber: false,
        showLoadCarrierHeaderItems: false,

        onActiveCompartmentChanged: (event: CompartmentType | undefined) =>
          onActiveCompartmentChanged(unref(pcotsLocationLeft), event),

        onViewContentClicked: () => onViewContentClicked(Position.left),

        onIdentifyLoadCarrier: (event: string) =>
          identifyLoadCarrierWithPosition(event, unref(pcotsLocationLeft)),

        onLoadCarrierAction: (event: LoadCarrierEvent) =>
          onLoadCarrierAction(
            unref(loadCarrierDataLeft),
            unref(pcotsLocationLeft),
            event
          ),
      },
    },
...
const targetLoadCarriersAisleRight = computed<PropsLivePickLoadCarriers[]>(
  () => [
    {
      ref: loadCarrierLeftRef,
      propsLivePickLoadCarrier: {
        headerTextOverride: 'B1',
        pcotsLocation: unref(pcotsLocationLeft),
        loadCarrier: unref(loadCarrierDataLeft),
        task: unref(showTask),
        position: Position.right,
        ...
        <div class="pick-center-one-ts">
          <div class="content">
            ...
            <div v-if="showLeftContainer" class="content-left">
              <LivePickAisle :load-carriers="targetLoadCarriersAisleLeft" />
            </div>
            ...
            <div v-if="showRightContainer" class="content-right">
              <LivePickAisle :load-carriers="targetLoadCarriersAisleRight" />
            </div>
            <div
              v-if="
          ...
          <div class="footer">
            ...
            <LivePickActionBar
              ...
              :is-action-bar-top-disabled="true"
              @action-button-clicked="onActionButtonClicked"
            />
          </div>
        </div>
